...
%% extract data of UK and Ireland
i = 1; j = 1; k = 1;
Data_UK = [];
Data_Ire = [];
while(i< len_text)
...
%Data_UK = movmean(Data_UK,7);   % Moving average mean of 3
Data_Ire = Data_Ire(1:size(Data_Ire,1)-1,:)  ; % -1 to remove the incomplete data
%Data_Ire = movmean(Data_Ire,7);
Data_Ire(1:3,:) = []; % due to Nan values
Data_UK(1:3,:) = []; % due to Nan values
A = Data_UK(:,5);
X1 = A(all(~isnan(A),2),:);  % UK daily deaths data
A = [];
A = Data_Ire(:,5);
X2 = A(all(~isnan(A),2),:);  % Ireland daily deaths data
...
% first wave
X1_Nep =  Data_UK(cpt_Nep(1):cpt_Nep(3),5);          % deaths in first wave
X1_Ind = Data_Ire(cpt_Ind(1):cpt_Ind(3),5);
...
% second wave
X2_Nep =  Data_UK(cpt_Nep(4):end,5);          % cases in first wave
X2_Ind = Data_Ire(cpt_Ind(4):cpt_Ind(end),5);
